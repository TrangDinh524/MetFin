...
import { AdvisorSection } from '../../features/advisor/AdvisorSection'
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-white text-[#0d1117]">
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <main className={`flex-1 overflow-y-auto bg-white px-7 py-5 ${base === 'advisor' ? 'flex flex-col' : ''}`}>
          {base === 'dashboard' && <DashboardSection />}
          {base === 'assets' && <AssetsSection />}
          {base === 'debt' && <DebtSection />}
          {base === 'scenarios' && <ScenarioSection />}
          {base === 'advisor' && <AdvisorSection />}
